...
d3.json("./static/stateMap/us.json", function(error, us) {
  ...
  var data = topojson.feature(us, us.objects.states).features;
    d3.csv("./static/statemap.csv", function(tsv){
...
d3.csv("./static/data/location.csv", function(data) {
  data.forEach(function(d) {
    d["city-state"] = d.Cname + ", " + d.Sname;
    ...
    d3.csv("./static/data/location.csv", function(data) {
      ...
      data.forEach(function(d) {
        for(i = 0; i < filters.length; i++ ) {
          if(filters[i].name == "climate") {
            if(filters[i].value == d[filters[i].name] && filters[i].checked == true) {
              raceFiltered_data.add(d);
            }
          }
          if(filters[i].name == "lighting") {
            if(filters[i].value == d[filters[i].name] && filters[i].checked == true) {
              genFiltered_data.add(d);
            }
          }
          if(filters[i].name == "drunk") {
            if(filters[i].value == d[filters[i].name] && filters[i].checked == true) {
              camFiltered_data.add(d);
            }
          }
          if(filters[i].name == "age") {
            if(filters[i].value == "Low" && filters[i].checked == true) {
              var age = +d["fatals"];
              if(0 <= age && age <= 1) {
                ageFiltered_data.add(d);
              }
            } else if(filters[i].value == "Mid" && filters[i].checked == true) {
              var age = +d["fatals"];
              if(2 <= age && age <= 3) {
                ageFiltered_data.add(d);
              }
            } else if(filters[i].value == "High" && filters[i].checked == true) {
              var age = +d["fatals"];
              if(age >= 4) {
                ageFiltered_data.add(d);
              }
  ...
  gPins.selectAll("circle")
    ...
    .attr("r",function(d) {
      if (!already_drawn_dot.has(d["city-state"])) {
        ...
        if(city_frequency[d["city-state"]] > 1){
    ...
    .on("mouseover", function(d) {
      ...
      tooltip.html("<strong>City: " + d["city-state"] + "</strong><br/>"
                  +"<strong>Accidents: " + city_frequency[d["city-state"]] + "</strong>")
    ...
    .on("click", (d) => {
      ...
      city_data.forEach((e) => {
        ...
        html_string += "<tr style='background: " + background + "'>"
                    + "<td class='date_col'>" + e.st_case + "</td>"
                    + "<td class='name_col'>" + e.Cname + "</td>"
                    + "<td class='race_col'>" + e.lighting + "</td>"
                    + "<td class='gender_col'>" + e.climate + "</td>"
                    + "<td class='age_col'>" + e.fatals + "</td>"
                    + "<td class='body_camera_col'>" + e.drunk + "</td>"
                    + "</tr>"
      })
...
var get_state_stats = function(state) {
  ...
  for (var i = 0; i < modified_data.length; i++) {
      if (modified_data[i].Sname === state) {
        counter++;
      }
  }
  return counter;
...
function clicked(d) {
  ...
  state_total
    ...
    .text(state_names[d.id] + " accidents: " +   get_state_stats(state_names[d.id]));
...
function sunburstDraw(scope, element) {
  ...
  function drawBreadcrumbTemplate() {
    ...
    while (bc < 4) {
      ...
      temp_breadcrumb
        ...
        .text(function() {              //#revisit
          if (bc === 0) { return "climate"; }
          if (bc === 1) { return "lighting"; }
          if (bc === 2) { return "fatals"; }
          return "drunk"
        });
  ...
  function mouseover(d) {
    ...
    // update summary
    summary.html(
      "<span class='percentage'>" + percentageString + "<br/> " + rawNumString + " accidents </span><br />"
